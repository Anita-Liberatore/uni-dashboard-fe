 ...
 * the sidebar component renders everything dynamically from here.
 */
...
export interface NavItem {
  ...
  /** Angular router path */
  route: string;
  /** Use exact matching for the active-link check (needed for '/') */
  exact?: boolean;
  /**
   * Heroicons 2.0 outline SVG path `d` attributes.
   * One entry per <path> element inside the icon SVG.
   */
  paths: string[];
  /** Optional badge count shown on the right side of the item */
  badge?: number;
}
